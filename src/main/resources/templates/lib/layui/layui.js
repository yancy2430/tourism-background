...
function reqPost(url, data, fun, err) {
    $.ajax({//异步请求返回给后台
        url: domain + url,
        type: 'POST',
        data: data,
...
function reqGet(url, data, fun, err) {
    $.ajax({//异步请求返回给后台
        url: domain + url,
        type: 'GET',
        data: data,
...
reqGet("admin/islogin", {}, function (res) {
    if (res.code == 0) {
        if (window.location.href.indexOf("/boss/login.html") > 0) {
    ...
    if (window.location.href.indexOf("boss/login")>=0){
